...
import { createDataTag, updateDatatag } from "../../../redux/tagSlice";
import * as Yup from "yup";
import { useDispatch } from "react-redux";

const CreateAndUpdateTag = ({ show, onHide, update }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (val) => {
      if (update) {
        await dispatch(updateDatatag({ id: update._id, name: val.name }));
      } else {
        await dispatch(createDataTag(val));
      }
      formik.resetForm();
      onHide();
    },
    validationSchema: Yup.object({
      name: Yup.string().required("nama tag tidak boleh kosong"),
    }),
  });
...
export default CreateAndUpdateTag;
